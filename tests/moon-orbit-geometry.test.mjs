...
assert.match(source, /id="c-eastern"/);
assert.match(source, /timeZone:'America\/New_York'/);
assert.doesNotMatch(source, /id="c-utc"|id="c-local"|\bUTC —|\bLocal —/);
...
assert.match(source, /document\.documentElement\.classList\.add\('mobile-layout'\)/);
